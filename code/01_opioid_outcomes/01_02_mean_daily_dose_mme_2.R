...
all_opioids_clean <- all_opioids_clean[setting %in% c("inpatient_acute", "other_acute"), .(BENE_ID, NDC, opioid, mme_strength_per_day, rx_start_dt, rx_end_dt)]
...
for (my_year in 2016:2019) {
  ...
  calculate_mean_daily_dose <- function(data) {
    ...
    ][, .(mean_daily_dose_mme_acute = mean(total_mme_strength))]
  }
  ...
  plan(sequential)
  out <- out |>
    mutate(mean_daily_dose_mme_acute = pmin(mean_daily_dose_mme_acute, quantile(mean_daily_dose_mme_acute,0.99)))
  toc()
  ...
  mean_daily_dose[, mean_daily_dose_mme_acute := fifelse(is.na(mean_daily_dose_mme_acute), 0, mean_daily_dose_mme_acute)]
  
  # Save final dataset -----------------------------------------------------------
  
  saveRDS(mean_daily_dose, file.path(save_dir, my_year, paste0(my_year, "_mean_daily_dose_mme_acute.rds")))
