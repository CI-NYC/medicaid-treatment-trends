...
for (my_year in 2016:2019) {
  ...
  otl <- readRDS(file.path(save_dir, my_year, paste0(my_year,"_otl_opioid_pain_rx.rds"))) |>
    filter(setting %in% c("inpatient_acute", "other_acute"))
  ...
  opioids$opioid_days_covered_acute <- 
  ...
  opioids <- select(dts_cohorts, BENE_ID) |> 
    ...
    mutate(opioid_days_covered_acute = replace_na(opioid_days_covered_acute, 0))
  
  # saveRDS(opioids, file.path(save_dir, "trends_proportion_days_covered.rds"))
  saveRDS(opioids, file.path(save_dir, my_year, paste0(my_year,"_proportion_days_covered_acute.rds")))
